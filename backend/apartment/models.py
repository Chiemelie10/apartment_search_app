...
class Apartment(models.Model):
    ...
    user_preferred_qualities = models.ManyToManyField(
        UserPreferredQuality, through='ApartmentUserPreferredQuality'
    )
...
class ApartmentUserPreferredQuality(models.Model):
    """
    This class defines the fields of the junction table between
    apartments table and user_preferred_qualities table.
    """
    id = models.CharField(default=uuid4, max_length=36,
                          unique=True, primary_key=True, editable=False)
    apartment = models.ForeignKey(Apartment, on_delete=models.CASCADE)
    user_preferred_quality = models.ForeignKey(UserPreferredQuality, on_delete=models.CASCADE)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    class Meta:
        ...
        ordering: The order the instances of this model is displayed on the admin page.
        """
        db_table = 'apartment_user_preferred_qualities'
        verbose_name_plural = 'Apartment user preferred qualities'
        ordering = ['-created_at']

    def __str__(self):
        """This method returns a string representation of the instance of this class."""
        # pylint: disable=no-member
        return f'Apartment_id: {self.apartment.id} - '\
                f'User_preferred_quality: {self.user_preferred_quality.name}'
